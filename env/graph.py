...
class Graph(object):
    ...
    def find_all_paths(self, start_vertex, end_vertex, path= [], max_depth=None):
        """ find all simple paths (path with no repeated vertices)
            from start vertex to end vertex in graph

            OPTIMIZATIONS:
            - max_depth: limit path length (default: num_vertices - 1)
            - Use set for O(1) membership testing
            - Early termination
        """
        if max_depth is None:
            max_depth = len(self.__graph_dict)

        # Convert path to list for building, but use set for lookup
        path = path + [start_vertex]

        if start_vertex == end_vertex:
            return [path]

        # Early termination: path too long
        if len(path) > max_depth:
            return []

        # Early termination: no outgoing edges
        if start_vertex not in self.__graph_dict or not self.__graph_dict[start_vertex]:
            return []

        paths = []
        path_set = set(path)  # O(1) membership testing

        for neighbor in self.__graph_dict[start_vertex]:
            if neighbor not in path_set:  # Much faster than list lookup
                sub_paths = self.find_all_paths(neighbor, end_vertex, path, max_depth)
                paths.extend(sub_paths)  # extend is faster than append in loop
...
class TrafficNetwork(Graph):
    ...
    def __init__(self, graph= None, O= [], D= [], max_paths_per_od=None):
        """
        ...
        max_paths_per_od : int, optional
            Limit paths per OD pair (e.g., 20) to prevent exponential explosion.
            If None, finds ALL paths (slow for dense networks)
        """
        Graph.__init__(self, graph)
        self.__origins = O
        self.__destinations = D
        self.__max_paths_per_od = max_paths_per_od
        self.__cast()
    ...
    def __cast(self):
        ...
        if self.__origins != None and self.__destinations != None:
            ...
            self.__paths, self.__paths_category = self.__generate_paths_by_demands(
                max_paths_per_od=self.__max_paths_per_od
            )
            ...
            for o in self.__origins:
                ...
                for d in self.__destinations:
                    self.paths_per_od[o][d] = []

            for path_idx, path_cat in enumerate(self.__paths_category):
                o, d = self.__OD_pairs[path_cat]
                self.paths_per_od[o][d].append(path_idx)
    ...
    def __generate_paths_by_demands(self, max_paths_per_od=None):
        """ According the demands, i.e. the origins and the
            ...
            PERFORMANCE: Set max_paths_per_od to limit paths (e.g., 20)
            to prevent exponential explosion on dense networks
        """
        paths_by_demands = []
        paths_category = []
        od_pair_index = 0
        ...
        for OD_pair in self.__OD_pairs:
            ...
            # Limit paths per OD pair if requested
            if max_paths_per_od is not None and len(paths) > max_paths_per_od:
                # Keep shortest paths (by number of hops)
                paths.sort(key=len)
                paths = paths[:max_paths_per_od]
        ...
        print(f"Path enumeration: {total_paths_found} paths for {len(self.__OD_pairs)} OD pairs")
